...
public class ClusteredLightning
{
        ...
        float nearCluster)
    {
        ...
        {
            g_LightsData[i] = new LightInfo(i >= visibleLights.Count ? null : visibleLights[i].light);
        }
        g_Lights.SetData(g_LightsData);
        
        // non-compute globals
        cmd.SetGlobalBuffer("g_Lights", g_Lights);
        cmd.SetGlobalInt("g_LightsCount", Mathf.Min(MAX_LIGHTS_PER_VIEW, visibleLights.Count));
        ...
        cmd.SetComputeIntParam(clusteredLightning, "g_LightsCount", Mathf.Min(MAX_LIGHTS_PER_VIEW, visibleLights.Count));
